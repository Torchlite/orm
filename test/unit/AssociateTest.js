...
assert(u._getSql(Team) === 'SELECT teams.team_id, teams.name, teams.owner FROM teams WHERE (team_id = 10)', `manyToOne get failed: ${u._getSql(Team)}`);
...
assert(u._getSql(Team, f) === `SELECT teams.team_id, teams.name, teams.owner FROM teams WHERE (team_id = 10) AND (name = 'a name')`, u._getSql(Team, f));
...
assert(u._getSql(Team, f, 10, 50, { key: 'teamId', direction: 'desc' }) === `SELECT teams.team_id, teams.name, teams.owner FROM teams WHERE (team_id = 10) AND (name = 'a name') ORDER BY team_id DESC LIMIT 10 OFFSET 50`, `limit/offset failed: ${u._getSql(Team, f, 10, 50, { key: 'teamId', direction: 'desc' })}`);
