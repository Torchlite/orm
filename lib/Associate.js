	...
	*/
	let Associate = Traits.define(['otherModel'], {
		...
		get: [function (otherModel, filter = {}) {
			...
			let order = filter.sort || null;

			delete filter.limit;
			delete filter.offset;
			delete filter.sort;

			let filterObject = otherModel.mapFields(filter);
			let trueFilterObject = {};

			Object.keys(filterObject).forEach(k => {
				trueFilterObject[`${otherModel.table.tableName}.${k}`] = filterObject[k];
			});

			return pool.query(this._getSql(
				otherModel,
				new Filter(trueFilterObject),
				limit,
				offset,
				order
			))
		...
		_getSql: [function (otherModel, filter, limit, offset, order) {
			...
			if (order) {
				let key = otherModel.fieldMap[order.key] || order.key;

				q.order(key, order.direction === 'asc');
			}
