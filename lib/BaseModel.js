...
class BaseModel {
	...
	static belongsToMany(otherModel, key, alias) {
		...
		this.associations.push(new Association(this, otherModel, 'manyToMany', key));
	}
		...
		switch (assoc._type) {
			case 'oneToMany':
				let coll = new (BaseCollection.of(otherModel))()
					.filter({
						[fm[assoc._key.references]]: this[assoc._key.key]
					});
			...
			case 'manyToMany':
				...
				let lTable = this.constructor.table.tableName;
				let otherFm = otherModel.fieldMap;

				class MNCollection extends BaseCollection.of(otherModel) {
					static get _baseSelector() {
						let s = super._baseSelector;
				...
				return new MNCollection()
					.filter({
						[`${lTable}.${fm[assoc._key.leftKey.key]}`]: this[assoc._key.leftKey.key]
					});

			case 'manyToOne':
				if (!this[assoc._key.key]) {
					throw new Error('Model doesn\'t have the necessary key!');
				}
