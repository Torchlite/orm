...
class BaseModel {
	...
	static hasMany(otherModel, key, alias) {
		...
		let assoc = new Association(this, otherModel, 'oneToMany', key);
		let pool = this._pool;
	...
	static belongsTo(otherModel, key, alias) {
		...
		let assoc = new Association(this, otherModel, 'manyToOne', key);
	...
	static belongsToMany(otherModel, key, alias) {
		...
		let assoc = new Association(this, otherModel, 'manyToMany', key);
		let thisModel = this;
				...
				class BelongsToManyCollection extends BaseCollection.of(otherModel) {
					static get _baseSelector() {
						...
						Object.keys(assoc._key.through.schema).forEach(k => {
							s.field(`${assoc._key.through.tableName}.${k}`, `__context_${k}`);
						});
	...
	*/
	...
	_cloneSql(propertiesToIgnore) {
		if (!propertiesToIgnore) {
			propertiesToIgnore = [];
		}

		let mappedToIgnore = propertiesToIgnore
			.filter(p => this.constructor.fieldMap[p])
			.map(p => this.constructor.fieldMap[p]);

		mappedToIgnore.push(this.constructor.table.pkey);

		let fields = Object.keys(this.constructor.table.schema)
			.filter(k => !mappedToIgnore.includes(k));

		let q = squel.insert()
			.into(this.constructor.table.tableName);

		fields
			.forEach(p => q.set(p, this[this.constructor._inverseFieldMap[p]] || null));

		Object.keys(this.constructor.table.schema)
			.forEach(k => {
				q.returning(k);
			});
	...
	async fetch() {
		return this._pool.query(this._fetchSql())
			.then(r => this.constructor.fromSQLRow(r.rows[0]));
	}

	async clone(propertiesToIgnore) {
		return this._pool.query(this._cloneSql())
			.then(r => this.constructor.fromSQLRow(r.rows[0]));
	}
	...
	*/
	async save() {
			return this._pool.query(this._saveSql())
	...
	*/
	async update(obj) {
		return this._pool.query(this._updateSql(obj))
			.then(r => new this.constructor(r.rows[0]));
	}
}
