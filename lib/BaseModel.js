...
class BaseModel {
			...
			switch (this.constructor.table.schema[k].type) {
				...
				case 'date':
					if (this[k] !== undefined && this[k] !== null) {
						obj[ifm[k]] = this[k].toISOString();
					} else {
						obj[ifm[k]] = null;
					}
					break;
				default:
					obj[ifm[k]] = this[ifm[k]];
					break;
			}
		});
	...
	*/
	...
	_cloneSql(overrides) {
		...
		fields
			.forEach(p => {
				if (mappedOverrides[p] === '__DEFAULT') {
					return q.set(p, 'DEFAULT', { dontQuote: true });
				}
				if (mappedOverrides[p]) {
					return q.set(p, mappedOverrides[p]);
				}
				return q.set(p, thisRow[this.constructor._inverseFieldMap[p]] || null);
			});
