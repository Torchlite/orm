...
class BaseCollection extends EventEmitter {
	...
	listen() {
		...
		let mapFields = this.constructor.associatedClass.mapFields;

		tab.on('UPDATE', msg => {
			let mappedNew = this.constructor.associatedClass.mapFields(msg.new);
			let mappedOld = this.constructor.associatedClass.mapFields(msg.old);

			if (
				this.filterObject.toJsFilter()(mappedOld) &&
				!this.filterObject.toJsFilter()(mappedNew)
			) {
				this.emit('remove', this.constructor.associatedClass.fromSQLRow(mappedOld));
			} else if (
				this.filterObject.toJsFilter()(mappedNew) &&
				!this.filterObject.toJsFilter()(mappedOld)
			) {
				this.emit('add', this.constructor.associatedClass.fromSQLRow(mappedNew));
			} else if (
				this.filterObject.toJsFilter()(mappedNew) &&
				this.filterObject.toJsFilter()(mappedOld)
			) {
				this.emit('update', this.constructor.associatedClass.fromSQLRow(mappedNew));
			}
		});
	...
	*/
	...
	get _baseSelector() {
		let mappedFields = this.constructor.associatedClass.mapFields(this.filterObject.obj);
...
/** Generate SQL to materialize this collection */
	toSql() {
		return this._collectSelector.toString();
	}
...
/** Count the number of items in the collection **/
	async count() {
		let result = await this.constructor._pool.query(this._countSql());
